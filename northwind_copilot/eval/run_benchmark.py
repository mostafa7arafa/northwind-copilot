...
Usage:
    python -m northwind_copilot.eval.run_benchmark                 # run all
    python -m northwind_copilot.eval.run_benchmark --id sql_employee_count_usa
    python -m northwind_copilot.eval.run_benchmark --limit 3
"""
...
from northwind_copilot.eval.graders import grade

# Harmless noise from langchain serializing ChatOllama state in the checkpointer.
warnings.filterwarnings("ignore", category=UserWarning, module="pydantic.main")

# run_benchmark.py lives at <repo>/northwind_copilot/eval/, so the repo root is
# three parents up. The dataset and the (gitignored) results dir sit at the root.
